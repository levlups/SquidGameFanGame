...
export function pasteBuild(x,y,z,facing){
	...
	for(const key in config){
			...
			if(config[key][1]==5 && config[key][2]==1 && config[key][3]==5){
				...
				makeEnt(noa,'cow',1,[config[key][1]+Math.round(x),config[key][2]+Math.round(y),config[key][3]+Math.round(z)])
			}
			// }, 500);
		}
	...
	for(const key in config){
			...
			if(config[key][1]==5 && config[key][2]==1 && config[key][3]==5){
				...
				makeEnt(noa,'cow',1,[config[key][3]+Math.round(x),config[key][2]+Math.round(y),config[key][1]+Math.round(z)-5])
			}
	...
	for(const key in config){
			...
			if(config[key][1]==5 && config[key][2]==1 && config[key][3]==5){
				...
				makeEnt(noa,'cow',1,[-config[key][3]+Math.round(x),config[key][2]+Math.round(y),-config[key][1]+Math.round(z)])
			}
		}
